...
const filteredWeatherList = computed(() => {
  ...
  if (keyword) {
    result = result.filter((item) => item.name.toLowerCase().includes(keyword) || item.name_kr.includes(keyword))
  }
  ...
  <div class="container">
    ...
    <div class="region-list">
      ...
      >
        ...
        <p>
          🌡️ 현재 기온:
          <span :style="{ color: item.temp >= HOT_TEMPERATURE ? 'red' : 'blue' }"> {{ item.temp }}°C </span>
        </p>

        <p>
          👤 체감온도:
          <span :style="{ color: item.main.feels_like >= HOT_TEMPERATURE ? 'red' : 'blue' }"> {{ Math.round(item.main.feels_like) }}°C </span>
        </p>
